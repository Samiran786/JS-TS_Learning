/*
==================================================================================
Arrays2_Print Subarray
Success rate: 44.16%
Given Array of length n and two integers start and end representing index. 
Print space separated Subarray of the Array from start idx to end idx.

Input Format:
The first line contains n representing length of Array
The second line represents n array elements
The third line contains 2 integers representing sp and ep.

Output Format:
Print space separated Subarray of the Array from start index to end index

Constraints:
1 <= N <= 20
1 <= arr[i] <= 10^9

Sample test cases
Input
5
10 20 30 40 50
2 4

Expected output

30 40 50 
==================================================================================
*/

//way 1 - 
let arr_sub:number[] = [10, 20, 30, 40, 50];

function subArrayPrinter(arr:number[], startIdx:number, endIdx:number):number[]{
    let subarr:number[] = [arr[startIdx]];
    let counter:number=1;
    for(let i=startIdx+1; i<=endIdx; i++ ){
        subarr[counter] = arr[i];
        counter++;
    }
    return subarr;
}

console.table(subArrayPrinter(arr_sub,2,4));

//Way 2 - 

function subArrayPrinterWay2(arr:number[], startIdx:number, endIdx:number):number[]{
    let subarr:number[] = [];
    for(let i=startIdx; i<=endIdx; i++ ){
        subarr.push(arr[i]);
    }
    return subarr;
}

console.table(subArrayPrinterWay2(arr_sub,2,4));